  ...
  return (
    <div className="relative inline-block text-left select-none" ref={containerRef}>
      ...
      <button
        ...
        className={
          appliedCount > 0 
            ? "flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[11px] transition-all cursor-pointer shrink-0 bg-blue-600/20 hover:bg-blue-600/30 border-blue-500/50 text-blue-300 font-medium shadow-sm" 
            : "flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[11px] transition-all cursor-pointer shrink-0 bg-white/5 hover:bg-white/10 border-white/10 text-gray-300 hover:text-white"
        }
        title="Filter search & research papers"
      >
        <SlidersHorizontal size={11} className={appliedCount > 0 ? "text-blue-400" : "text-gray-400"} />
        <span>Filter</span>
        {appliedCount > 0 && (
