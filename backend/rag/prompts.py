...
def get_workspace_analysis_system_prompt(doc_count: int) -> str:
    return (
        ...
        "STRICT STOPPING RULE (NO UNSOLICITED SECTIONS & NO CITATION DUPLICATION):\n"
        ...
        "- DO NOT DUPLICATE CITATIONS: Place at most ONE citation tag per paper (e.g. write '1. [5] Title (Year)'. NEVER repeat '[5]' again at the end of the line).\n\n"
        "CITATION CLEANLINESS & NO BUTTON SPAM (CRITICAL):\n"
        "- Each citation badge [X] is automatically rendered by the platform UI as an interactive clickable button.\n"
        "- NEVER spam dozens of consecutive citation numbers in prose (e.g. NEVER write '[1], [2], [3], [4], [5], ... [20]' in a paragraph, as this renders as an overwhelming row of 20 buttons causing severe visual clutter).\n"
        "- For categorizations or list items, simply identify each paper cleanly (e.g. '1. [1] Paper Title (Year)') without repeating chains of bracketed numbers in summary sentences.\n"
        "- Attach citations [X] ONLY beside specific empirical findings, statistics, or unique methodology claims that genuinely benefit from source verification.\n\n"
        ...
        "AI CITATION GROUNDING MAP:\n"
        "At the VERY END of your response, if you cited specific empirical findings or quotes, append a hidden JSON metadata block.\n"
        "For EACH cited document number [X] whose findings are discussed, extract the EXACT verbatim sentence(s) directly from the source document text that contain the specific claim, method, or metric cited.\n"
        ...
        "- EXEMPTION (CRITICAL): If your response is simply listing, enumerating, counting, or categorizing the documents in the workspace (like listing full-text vs abstract status) without asserting specific empirical paper results, DO NOT generate a CITATION_MAP block at all.\n\n"
        "Format strictly as:\n"
        "<!-- CITATION_MAP: {\n"
