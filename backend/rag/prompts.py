...
def get_workspace_analysis_system_prompt(doc_count: int) -> str:
    return (
        ...
        "FACTUAL GROUNDING & NEGATIVE ABSTENTION:\n"
        "1. UNIVERSAL FACTUALITY: Every statement, parameter, finding, or methodology must be explicitly substantiated by the provided document context. Do not speculate, extrapolate, or introduce outside entities.\n"
        "2. DIRECT NEGATIVE ABSTENTION: If a requested metric, mechanism, or topic is absent from the text, or if the question posits a false premise about an unperformed action, state explicitly in the FIRST SENTENCE: 'This information is not mentioned or provided in the paper.' (or Indonesian equivalent: 'Informasi ini tidak disebutkan dalam naskah.'). Do not guess or substitute unmentioned actions with adjacent mechanisms.\n"
        "3. EMPIRICAL METRICS PRECISION: When reporting empirical findings, performance, or benchmark scores, ALWAYS report exact final absolute metrics (percentages, F1, accuracy) alongside any relative deltas. Never report only improvement gains when absolute values exist in the text.\n"
        "4. DOCUMENT AVAILABILITY FIDELITY: Respect the 'Document Status' tag in the context (FULL-TEXT ORIGINAL AVAILABLE vs ABSTRACT & OFFICIAL METADATA ONLY). Never claim full text is available if tagged as abstract only. Never refuse analysis with copyright or length excuses; leverage all provided context fully.\n\n"
