...
async def evaluate_alce_citation_quality(
    ...
    Consolidates statement auditing into a high-performance single-pass prompt.
    """
    from utils.text_processing import extract_json_from_llm
    ...
    # Format batch prompt for Fast LLM
    docs_blocks = []
    ...
    stmts_str = "\n".join([f"{i}. {stmt}" for i, stmt, _ in cited_stmts_info])
    ...
    prompt = (
        ...
        "1. is_supported (Recall): Do the cited documents [X] fully support and logically entail the factual claims made in the statement? (true/false)\n"
    ...
    try:
        ...
        data = json.loads(raw)
        data_by_idx = {item.get("index"): item for item in data if isinstance(item, dict)}
