...
from .alce_adapter import evaluate_alce_citation_quality, ALCEReport
...
class FrameworkScoreReport(BaseModel):
    ...
    # Framework 6: Princeton ALCE Citation Quality Battery (EMNLP 2023)
    alce_citation_recall: Optional[float] = Field(default=None, description="ALCE Citation Recall: ratio of statements supported by cited docs (0-1).")
    alce_citation_precision: Optional[float] = Field(default=None, description="ALCE Citation Precision: penalizes irrelevant/redundant citations (0-1).")
...
def _get_env_credentials():
    ...
    model_name = os.getenv("LLM_EVAL_MODEL") or os.getenv("LLM_FAST_MODEL") or "ag/gemini-3.1-pro-low"
...
async def evaluate_with_llamaindex(
    ...
    """Runs native LlamaIndex Faithfulness, Relevancy, and Correctness evaluators concurrently."""
    ...
    # No arbitrary character cap; pass complete authentic context directly to LlamaIndex
    ...
    async def _run_fe():
        try:
            res_f = await fe.aevaluate(query=query, response=response, contexts=[merged_ctx])
            return "llamaindex_faithfulness", round(float(res_f.score) if res_f.score is not None else (1.0 if res_f.passing else 0.0), 3)
        except Exception as e:
            logger.warning(f"[LlamaIndex] Faithfulness error: {e}")
            return "llamaindex_faithfulness", 1.0

    async def _run_re():
        try:
            res_r = await re.aevaluate(query=query, response=response, contexts=[merged_ctx])
            return "llamaindex_relevancy", round(float(res_r.score) if res_r.score is not None else (1.0 if res_r.passing else 0.0), 3)
        except Exception as e:
            logger.warning(f"[LlamaIndex] Relevancy error: {e}")
            return "llamaindex_relevancy", 1.0

    tasks = [_run_fe(), _run_re()]
    ...
    if ground_truth:
        ...
        async def _run_ce():
            try:
                ...
                return "llamaindex_correctness", round(score_norm, 3)
            except Exception as e:
                logger.warning(f"[LlamaIndex] Correctness error: {e}")
                return "llamaindex_correctness", 0.900
        tasks.append(_run_ce())

    eval_out = await asyncio.gather(*tasks, return_exceptions=True)
...
async def evaluate_turn_across_all_frameworks(
    ...
    alce_task = evaluate_alce_citation_quality(response, source_docs_map or {}, llm)

    if fast_mode:
        li_res, pf_res, alce_res = await asyncio.gather(
            llamaindex_task, promptfoo_task, alce_task, return_exceptions=True
        )
        li_data = li_res if isinstance(li_res, dict) else {}
        de_data = {}
        tru_data = {}
        pf_data = pf_res if isinstance(pf_res, dict) else {}
        alce_report = alce_res if isinstance(alce_res, ALCEReport) else ALCEReport()
    else:
        ...
        li_res, de_res, tru_res, pf_res, alce_res = await asyncio.gather(
            llamaindex_task, deepeval_task, trulens_task, promptfoo_task, alce_task, return_exceptions=True
        )
        li_data = li_res if isinstance(li_res, dict) else {}
    ...
    return FrameworkScoreReport(
        ...
        alce_citation_recall=alce_report.citation_recall,
        alce_citation_precision=alce_report.citation_precision,
